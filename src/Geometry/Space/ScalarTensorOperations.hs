...
----------------------------------------------------------------------------------
--
--instance ScalarTensor (Scalar t) t where
--    fromScalar x = Scalar x
--
--instance (Num t) => ScalarTensorNum (Scalar t) t where
--    c ..* (Scalar x) = Scalar (c*x)
--    (Scalar a) .*. (Scalar p) = a*p
--
--instance (Fractional t) => ScalarTensorFractional (Scalar t) t where 
...
----------------------------------------------------------------------------------
--
--instance ScalarTensor [[t]] t where
--    fromScalar x = [[x]]
--
--instance (ScalarTensor (s t) t) => ScalarTensor [s t] t where
--    fromScalar x = [fromScalar x]
--
--instance (Num t) => ScalarTensorNum [[t]] t where
--    (..*) = map . map . (*)
--    x .*. y = sum $ zipWith (\xs ys -> sum $ zipWith (*) xs ys) x y
--
--instance (ScalarTensorNum (s t) t) => ScalarTensorNum [s t] t where
--    (..*) = map . (..*)
--    x .*. y = sum $ zipWith (.*.) x y
--
--instance (Fractional t) => ScalarTensorFractional [[t]] t where
--    xs /.. x = map (map (/x)) xs
--    (../) = map . map . (/)
--
